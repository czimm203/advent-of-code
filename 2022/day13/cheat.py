...
input_packets = """[[[[4,7,7],0,4,[6,3,3,7,10],2],2,[[3]]],[]]
[[[[1,4,8],[3,3,1,4]]]]
...
def is_ordered(left, right):
    if isinstance(left, int) and isinstance(right, int):
        ...
        r = is_ordered(left[0], right[0])
        return r if r != 0 else is_ordered(left[1:], right[1:])


sum = 0
packets = [packet.split('\n') for packet in input_packets.split('\n\n')]
for index, packet_pair in enumerate(packets):
    packet_pair = (json.loads(packet_pair[0]),
                   json.loads(packet_pair[1]))
    if is_ordered(*packet_pair) < 0:
        print(*packet_pair,index+1,"\n", sep="\n")
        sum += index + 1
print(sum)

packets = [json.loads(packet) for packet in input_packets.rstrip().split('\n') if len(packet) > 0]
start, end = [[2]], [[6]]
...
print(
    (packets.index(start) + 1) * (packets.index(end) + 1)
)
